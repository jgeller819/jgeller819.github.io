var EquilibriumGraph = Graph.extend({
  ...
  defaults: function() {
    return {
      ...
      "x_intercept": true, //show
      "x_intercept_texture": 1, // solid/dashed
      "x_intercept_label": "Q* = ",
      "x_intercept_color": "brown",

      "y_intercept": true, //show
      "y_intercept_texture": 1, // solid/dashed
      "y_intercept_label": "Q* = ",
...
generateProducerSurplus: function() {
  ...
  var position = {};
      ...
      processTouch: function() {
        ...
        if (activeLine === 0 || activeLine == 4) {
            ...
            if (!params['parabola']) {
              ...
              params['s_int_inv'] = s_int_inv / s_slope_inv;
            }
            else {
        ...
        removeShift: function(lineIndex) {
          if (lineIndex === 0) {
            // Remove the last demand shift
            var shifts = this.get("demandShiftCount");
            shifts = Math.max(shifts - 1, 0);
            this.set("demandShiftCount", shifts);
            this.rebuildGraph(true);
          }
          else if (lineIndex === 4) {
       ...
       generateDemandLine: function(fill, show, shiftIndex) {
        ...
        return new Line({
          active: show,
          points: points,
          fillOn: params["shade"] && fill,
          fillColor: params["demand_fill_color"],
          lineColor: params["demand_color"],
          lineLabel: params["demand_label_text"] + Array(shiftIndex + 1).join("'"),
          showLineLabel: params["demand_label"] && show,
      ...
      return new Line({
        ...
        lineLabel: params["supply_label_text"] + Array(shiftIndex + 1).join("'"),
        showLineLabel: params["supply_label"]  && show,
        lineLabelYPosition: 0,
        lineLabelXPosition: "right"
      });
    },
    ...
    generateCenterLines: function() {
      ...
      var pLine1 = new Line({
        ...
        fillAndStroke: false,
        fillColor: params["supply_fill_color"],
    ...
    updateLocalVariables: function() {
      ...
      if (!params["parabola"]) {
        ...
        var s_slope = pS / (qS - params["s_vertex"]);
        var s_int = pS - (pS * qS) / (qS - params["s_vertex"]);
        var s_int_p = pS - (pS * qS) / (qS - this.get("s_vertex'"));
        var s_int_pp = pS - (pS * qS) / (qS - this.get("s_vertex''"));

        var d_slope = -pS / (qS - params["d_vertex"]);
        var d_int = pS - (pS * qS) / (qS - params["d_vertex"]);
        var d_int_p = pS - (pS * qS) / (qS - this.get("d_vertex'"));
        var d_int_pp = pS - (pS * qS) / (qS - this.get("d_vertex''"));

        params["d_slope_inv"] = 1 / d_slope;
        params["s_slope_inv"] = 1 / s_slope;
        params["d_int_inv"] = d_int / d_slope;
        this.set("d_int_inv'", d_int_p / d_slope);
        this.set("d_int_inv''", d_int_pp / d_slope);

        params["s_int_inv"] = s_int  / s_slope;
        this.set("s_int_inv'", s_int_p / s_slope);
        this.set("s_int_inv''", s_int_pp / s_slope);
      }

      var maxX = params["maxXValue"];
      var maxY = params["maxYValue"];
      var minX = params["minXValue"];
      var minY = params["minYValue"];
      var xString = roundedString(this.get("q*"), maxX);
      var yString = roundedString(this.get("p*"), maxY);
      this.set("xAxisTicks", [[minX, "" + roundedString(minX, maxX)], [this.get("q*"), params["x_intercept_label"] + xString], [maxX, "" + roundedString(maxX, maxX)]]);
      this.set("yAxisTicks", [[minY, "" + roundedString(minY, maxY)], [this.get("p*"), params["y_intercept_label"] + yString], [maxY, "" + roundedString(maxY, maxY)]]);
    },
